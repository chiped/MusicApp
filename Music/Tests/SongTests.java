...
public class SongTests {
	...
	public void noteFromStringTest() {
		...
		String n2String = n2.toString();
		String n3String = n3.toString();
		assertEquals("Note {duration=0.0:pitch=[0]:arrivalTime=0.0}", Note
				.makeNoteFromString(n1String).toString());
		assertEquals("Note {duration=1.0:pitch=[1]:arrivalTime=1.0}", Note
				.makeNoteFromString(n2String).toString());
		assertEquals("Note {duration=2.0:pitch=[2]:arrivalTime=2.0}", Note
				.makeNoteFromString(n3String).toString());
	}
	...
	public void multiNoteToStringTest() {
		...
		assertEquals("Note {duration=0.0:pitch=[0]:arrivalTime=0.0}", n1String);
		assertEquals("Note {duration=1.0:pitch=[0, 1]:arrivalTime=1.0}",
				n2String);
		assertEquals("Note {duration=2.0:pitch=[0, 1, 2]:arrivalTime=2.0}",
				n3String);
	}
	...
	public void multiNoteFromStringTest() {
		...
		assertEquals("Note {duration=0.0:pitch=[0]:arrivalTime=0.0}", Note
				.makeNoteFromString(n1String).toString());
		assertEquals("Note {duration=1.0:pitch=[0, 1]:arrivalTime=1.0}", Note
				.makeNoteFromString(n2String).toString());
		assertEquals("Note {duration=2.0:pitch=[0, 1, 2]:arrivalTime=2.0}",
				Note.makeNoteFromString(n3String).toString());
	}
	...
	public void songToStringTest() {
		...
		String s1StringTest = "Song (songName=Worst Of Lullabies;instrument=Guitar;notes="
				+ notesString + ";key=[A, B, C#];genre=Rock and Roll)";
